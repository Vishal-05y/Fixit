...
import { getAllServiceCategories } from "@/data/homeservices";
import Link from "next/link";
    ...
    return (
        <div className="flex flex-wrap justify-center gap-6 p-8">
            {categories.map((category) => (
                <Link key={category} href={`/homeservices/category/services?category=${encodeURIComponent(category)}`}>
                    <div className="w-64 bg-white shadow-lg rounded-2xl p-6 text-center transform transition duration-300 hover:scale-105 cursor-pointer">
